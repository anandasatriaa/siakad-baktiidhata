                    ...
                    <div class="col-md-6">
                        <div class="form-group mb-3">
                            ...
                            <select id="tahun_akademik_id" class="form-select @error('tahun_akademik_id') is-invalid @enderror" name="tahun_akademik_id" required>
                                @foreach ($periodes as $p)
                                    <option value="{{ $p->id }}" {{ old('tahun_akademik_id', $keterlambatan->tahun_akademik_id) == $p->id ? 'selected' : '' }}>
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select id="siswa_id" class="form-select @error('siswa_id') is-invalid @enderror" name="siswa_id" required>
                                ...
                                @foreach ($siswas as $s)
                                    <option value="{{ $s->id }}" {{ old('siswa_id', $keterlambatan->siswa_id) == $s->id ? 'selected' : '' }}>
                                        {{ $s->nama_lengkap }} ({{ $s->riwayatKelas->where('tahun_akademik_id', $keterlambatan->tahun_akademik_id)->first()->kelas->nama_kelas ?? '-' }})
                                    </option>
                                @endforeach
                            </select>
